...
allowed-tools: read-file list-directory run-command write-file
---
...
1. **Information Gathering:**
   - Use `run-command` to execute Python-based static analysis. Prefer standard static analysis tools. If no tools like `bandit` or `pylint` are installed, use a fallback `run-command` execution with `grep` to scan for common vulnerabilities (e.g., hardcoded "password", "secret", "TODO", "FIXME", `eval()`, etc).
   - Use `list-directory` and `read-file` to examine critical code segments if static analysis flags suspicious files.
...
3. **Reporting:**
   - Use `write-file` to generate a report named `autopsy-report.md`.
